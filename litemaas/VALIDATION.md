...
After running `./preparation.sh`, the three secret YAML files are filled from templates. Verify they exist and have no leftover template variables:

```bash
ls -la backend-secret.yaml postgres-secret.yaml litellm-secret.yaml kustomization.yaml
```
...
Check that template variables were replaced (should see no `${...}` patterns in the secret files):

```bash
grep '\${' backend-secret.yaml postgres-secret.yaml litellm-secret.yaml 2>/dev/null || true
```

**Expected output:** (empty - no matches)

If you see any `${VARIABLE_NAME}` patterns, the variable was not set in `user-values.env` when you ran `preparation.sh`.
