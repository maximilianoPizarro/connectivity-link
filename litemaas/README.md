...
2. **Cluster domain** (single source): Edit `cluster-config.env` or run from the **repo root**:
   ```bash
   ./update-cluster-domain.sh apps.your-cluster.example.com
   ```
   This updates `cluster-config.env`, `OAuthClient.yaml`, and `litemaas-gateway.yaml`.

3. **Edit `user-values.env`** for **secrets only** (no domain here):
...
4. **Run the preparation script** to generate the three secret YAML files from templates (overwrites placeholders):
...
5. **Verify**:
   ...
   ```bash
   ls -la backend-secret.yaml postgres-secret.yaml litellm-secret.yaml
   ```

> **Domain**: Cluster-dependent URLs live in `cluster-config.env` and are loaded into the `litemaas-cluster-config` ConfigMap. Deployments and routes use this; `OAuthClient.yaml` and `litemaas-gateway.yaml` are updated by `update-cluster-domain.sh` from the repo root. **Secrets**: Run `preparation.sh` to fill `backend-secret.yaml`, `postgres-secret.yaml`, and `litellm-secret.yaml` from `user-values.env`; deployments and namespace are static YAML.
